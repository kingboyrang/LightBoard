...
-(NSArray*)selectNodes:(NSString*)xpath forObject:(Class)cls outNodes:(NSArray**)outnodes{
    return [self selectNodes:xpath namespaces:nil forObject:cls outNodes:outnodes];
}
-(NSArray*)selectNodes:(NSString*)xpath namespaces:(NSDictionary*)spaces forObject:(Class)cls outNodes:(NSArray**)outnodes{
    if (self.document) {
        NSMutableArray *array=[NSMutableArray array];
        GDataXMLElement* rootNode = [self.document rootElement];
        NSArray *childs;
...
-(NSArray*)selectChildsNodesToObjects:(GDataXMLNode*)node xpath:(NSString*)path forObject:(Class)cls{
    assert((node == nil) == (path == nil) );
    NSMutableArray *result=[NSMutableArray array];
    NSArray *childs=[node nodesForXPath:path error:nil];
    for (GDataXMLNode *item in childs){
